...
C     p confidence interval for any number of degrees of freedom.
      ...
      q = 1 - p
      hdof = dble(ndof)/2.0D0

      fmid = real(gamma_q( hdof, dble(x2)/2.0D0 )) - q
      f = real(gamma_q( hdof, dble(x1)/2.0D0 )) - q
      ...
      do 11 j=1,JMAX
         dx = dx*0.5
         xmid = dchi + dx
         fmid = real(gamma_q( hdof, dble(xmid)/2.0D0 )) - q
 ...
 11   continue
      ...
      double precision function gamma_q( a, x )
...
!     +   1, 1)
      if ( x.eq.0.D0) then
         write(*,*) "dgamma: Fatal Error:  X is 0.0"
         stop
      endif
      if ( x.lt.0.D0 .and. x+n-2 .eq.0.D0) then
         write(*,*) "dgamma: Fatal Error:  X is a negative integer"
         stop
      endif
      if ( x.lt.-0.5D0 .and. abs((x-aint(x-0.5D0))/x) .lt. dxrel ) 
     x     write(*,*) "dgamma: Warning:  X is near negative integer"
...
!     +   'X SO BIG GAMMA OVERFLOWS', 3, 2)
 50   if ( x.gt.xmax ) then
         write(*,*) "dgamma: Overflow Error:  X too large"
         stop
      endif
C
      DGAMMA = 0.D0
...
!      IF (X.LT.XMIN) RETURN
      if ( x.lt.xmin ) then
         write(*,*) "dgamma: Underflow Error:  X too small"
         return
      endif
C
...
!     +   'ANSWER LT HALF PRECISION, X TOO NEAR NEGATIVE INTEGER', 1, 1)
      if ( abs((x-aint(x-0.5D0))/x) .lt. dxrel ) 
     x     write(*,*) "dgamma: Warning:  X near negative integer value"
C
      SINPIY = SIN (PI*Y)
...
!     +   , 2, 2)
      if ( x.lt.0.D0) then
         write(*,*) "dgamit: Fatal Error:  X is negative."
         stop
      endif
C
...
!     +   1)
      write(*,*) "dgamit: Warning:  result is less than half precision."
C
...
!     +   'X IS A NEGATIVE INTEGER', 3, 2)
      if ( sinpiy.eq.0.D0) then
         write(*,*) "dlngam: Fatal Error:  X cannot be negative."
         stop
      endif
C
!      IF (ABS((X-AINT(X-0.5D0))/X) .LT. DXREL) CALL XERMSG ('SLATEC',
...
!     +   1, 1)
      if (abs((x-aint(x-0.5D0))/x) .lt. dxrel ) 
     x     write(*,*) "dlngam: Warning:  X is very near to a "//
...
!     +   'Number of coefficients is less than 1', 2, 1)
      if ( nos.lt.1 ) write(*,*) "initds: Error:  Number of "//
...
!     +   'Chebyshev series too short for specified accuracy', 1, 1)
 20   if ( i.eq.nos ) 
     x     write(*,*) "initds: Error:  Chebyshev series too short"
      INITDS = I
C
      RETURN
      END
...
!      CALL XERMSG ('SLATEC', 'DGAMLM', 'UNABLE TO FIND XMIN', 1, 2)
      write(*,*) "dgamlm: Fatal Error:  Unable to find xmin"
      stop
C
 20   XMIN = -XMIN + 0.01D0
C
...
!      CALL XERMSG ('SLATEC', 'DGAMLM', 'UNABLE TO FIND XMAX', 2, 2)
      write(*,*) "dgamlm: Fatal Error:  Unable to find xmax"
      stop
C
 40   XMAX = XMAX - 0.01D0
...
!     +   'X OUTSIDE THE INTERVAL (-1,+1)', 1, 1)
      if ( n.lt.1 ) then
         write(*,*) "dcsevl: Fatal Error:  Num. of terms less than 1"
         stop
      endif
      if ( n.gt.1000) then 
         write(*,*) "dcsevl: Fatal Error:  Num. of terms greater "//
     x        "than 1000"
         stop
      endif
      if ( abs(x).gt.onepl ) 
     x     write(*,*) "dcsevl: Error:  X outside interval (-1,+1)"
C
      B1 = 0.0D0
      B0 = 0.0D0
      TWOX = 2.0D0*X
...
C 20040518  Removed ancient hardware (left IBM PC) (JH)
C 20040518  Removed "EQUIVALENCE" lines (JH)
...
C
C
C *** JH:: NOTE: I removed all constants except for IBM PC
C *** JH::       If these are not right for your architecture, 
C *** JH::       just change the values manually.
...
C     ON 8088, I.E., NOT IN 80 BIT FORM FOR THE 8087.
      ...
      dmach(3) = 1.11D-16   
      dmach(4) = 2.22D-16   
      dmach(5) = 0.301029995663981195D0 

C***FIRST EXECUTABLE STATEMENT  D1MACH
!      IF (I .LT. 1 .OR. I .GT. 5) CALL XERMSG ('SLATEC', 'D1MACH',
!     +   'I OUT OF BOUNDS', 1, 2)
      if ( i.lt.1.or.i.gt.5 ) then
         write(*,*) "d1mach: Fatal Error:  i out of bounds"
         stop
      endif
C
      D1MACH = DMACH(I)
...
!     +   'X MUST BE GE 10', 1, 2)
      if ( x.lt.10.0D0 ) then
         write(*,*) "d9lgmc: Fatal Error:  X is less than 10."
         stop
      endif
...
!     +   1)
      write(*,*) "d9lgmc: Underflow Error:  X too large"
      RETURN
C
      END
...
!     +   'NO CONVERGENCE IN 200 TERMS OF TAYLOR-S SERIES', 2, 2)
      write(*,*) "d9gmit: Fatal Error:  no convergence after 200 "//
...
!     +   'NO CONVERGENCE IN 300 TERMS OF CONTINUED FRACTION', 1, 2)
      write(*,*) "d9lgic: Fatal Error:  No convergence after 300 "//
...
!     +   'X SHOULD BE GT 0.0 AND LE A', 2, 2)
      if ( x.le.0.0D0 .or. a.lt.x ) then
         write(*,*) "d9lgit:  Fatal Error:  "//
...
!     +   'NO CONVERGENCE IN 200 TERMS OF CONTINUED FRACTION', 3, 2)
      write(*,*) "d9lgit: Fatal Error:  No convergence after 200"//
...
!     +   'RESULT LESS THAN HALF PRECISION', 1, 1)
      if ( hstar.lt.sqeps ) write(*,*) "d9lgit:  Warning:  Result is"//
